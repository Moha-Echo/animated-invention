...
RunService.RenderStepped:Connect(function() 
    ...
    if CTX.aimbotActive then
        ...
        if CTX.trackedTarget
            and not isEnemyValidForTracking(CTX.trackedTarget)
        then
            resetAimPart(CTX.trackedTarget)
            CTX.trackedTarget = nil
            CTX.currentTarget = nil
...
local function updateAITarget(hrp)
    ...
    if CTX.AITarget
        and isDuelTargetActive(CTX.AITarget)
    then
        CTX.AITargetLastValid = now
        return CTX.AITarget
    end

    if now - CTX.AILastAcquire < 0.10 then
        return CTX.AITarget
    end
...
local function processCamera(targetEntity)
    ...
    if not nativeAimHasPriority
        and targetEntity
        and canAimAtEntity(targetEntity)
    then
