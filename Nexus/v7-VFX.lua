...
local function injecterCouteauLance(thrownKnife)
    ...
    if CTX.Skin.EnabledModel then
        ...
        if modelAlternatif then
            ...
            -- aucune pièce physique originale n'est supprimée.
            -- Seul son rendu local est masqué.
            ...
            -- Le modèle custom est uniquement visuel.
            for _, newItem in pairs(modelAlternatif:GetChildren()) do
                if newItem ~= modelAlternatif:FindFirstChild("Center")
                    and newItem:IsA("BasePart")
                then
                    local cloneItem = newItem:Clone()

                    cloneItem.CanCollide = false
                    cloneItem.CanTouch = false
                    cloneItem.CanQuery = false
                    cloneItem.Massless = true
                    cloneItem.Anchored = false
...
local function isTargetCurrentAndAlive(entity)
    if not entity then
        return false
    end
    ...
    local aimPart = getAimPart(entity)
    if not aimPart then
        return false
    end

    return aimPart:IsDescendantOf(character)
end
...
local function aimAtTarget(entity)
    ...
    if not isTargetCurrentAndAlive(entity) then
        resetAimPart(entity)
        return
    end
...
RunService.RenderStepped:Connect(function() 
    ...
    if CTX.aimbotActive then
        -- Une cible native morte/invalide est retirée immédiatement.
        if CTX.trackedTarget
            and not isTargetCurrentAndAlive(CTX.trackedTarget)
        then
        ...
        if tick() - CTX.lastTargetTime >= CTX.AimbotPriorityConfig.RecheckInterval then
        ...
        -- L'Aimbot natif possède la priorité absolue de caméra.
        if CTX.trackedTarget
            and isTargetCurrentAndAlive(CTX.trackedTarget)
            and canAimAtEntity(CTX.trackedTarget)
        then
            aimAtTarget(CTX.trackedTarget)
        end

        -- Auto Shoot : Wallbang peut maintenir le lock,
        -- mais le tir exige une visibilité réelle.
...
CTX.CombatSection:AddSlider({ Name = "Cooldown M2", Min = 0.1, Max = 1, Default = 0.15, Increment = 0.05, Callback = function(v) CTX.AutoPlayConfig.AttackCooldownM2 = v end })
...
local function updateAITarget(hrp)
    ...
    -- Une cible IA morte ou devenue invalide est retirée immédiatement.
    if CTX.AITarget
        and not isTargetCurrentAndAlive(CTX.AITarget)
    then
...
local function processCamera(targetEntity)
    ...
    -- Une cible IA invalide ne doit jamais reprendre la caméra.
    if not isTargetCurrentAndAlive(targetEntity) then
        return
    end
