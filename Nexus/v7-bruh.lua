...
local function runIALogic(hybridMode)
    ...
    if targetChar then
        local hp = tonumber(targetChar:GetAttribute("HP")) or 0
        local elim = targetChar:GetAttribute("DuelEliminated")
        if elim or hp <= 0 then
...
RunService.Heartbeat:Connect(function()
    ...
    if CTX.AITarget then
        if not isEntityAlive(CTX.AITarget) then
            resetAimPart(CTX.AITarget)   -- on appelle que si l'entité existe
            CTX.AITarget = nil
            CTX.AITargetLastVisible = false
        end
    end

    if CTX.trackedTarget then
        if not isEntityAlive(CTX.trackedTarget) then
            resetAimPart(CTX.trackedTarget)
            CTX.trackedTarget = nil
            CTX.currentTarget = nil
        end
