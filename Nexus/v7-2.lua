...
-- Runtime singleton : nettoie l'ancienne instance de Nexus V7 avant re-exécution.
if _G.NexusV7Runtime and _G.NexusV7Runtime.Shutdown then
    pcall(function() _G.NexusV7Runtime.Shutdown() end)
end
...
local function injecterCouteauLance(thrownKnife)
    ...
    if CTX.Skin.EnabledModel then
        ...
        if modelAlternatif then
            ...
            visualModel.Name = "NexusKnifeVisual"
...
getAimPart = function(entity)
    if not entity then return nil end   -- <-- ajouter cette ligne
    local character = getEntityCharacter(entity)
...
CTX.ESPTab:AddToggle({ Name = "Tracers", Default = true, Callback = function(v) _G.TracersEnabled = v if not v then cleanupAllTracers() end end })
