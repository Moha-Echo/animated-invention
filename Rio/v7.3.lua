...
local function actualiserViewmodel()
    ...
    -- Mise à jour du placement du modèle custom + offsets manuels.
    if currentKnife and currentKnife:GetAttribute("CustomModel") == true then
        local jointPart = targetViewModel:FindFirstChild("Right Arm")
        local handle = currentKnife:FindFirstChild("Handle") or currentKnife:FindFirstChild("Main")
        local motor = handle and handle:FindFirstChild("KnifeMotor")
        ...
        if jointPart and handle then
            ...
            local offset = CTX.Skin.ManualOffset or Vector3.zero
            local rotation = CTX.Skin.ManualRotation or Vector3.zero

            motor.C0 =
                CFrame.new(cfg.Position + offset)
                * cfg.Rotation
                * CFrame.Angles(
                    math.rad(rotation.X),
                    math.rad(rotation.Y),
        ...
        if CTX.Skin.EnabledSkin then
            if CTX.Skin.Knife == "Butterfly" then
                ...
                if sourceTexture then
                    for _, obj in pairs(mainMesh:GetChildren()) do
                        if obj:IsA("SurfaceAppearance") or obj:IsA("Texture") then
                            obj:Destroy()
                        end
                    end
