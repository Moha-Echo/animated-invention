...
for _, guiName in ipairs({"rio v2.0", "rio v3.0", "rio v4.0",  "rio v5.0", "rio v7.0"}) do
...
local function actualiserViewmodel()
    ...
    if mainMesh then
        ...
        if CTX.Skin.EnabledSkin then
            ...
            local textureName = CTX.Skin.Texture

            if type(textureName) == "string"
                and textureName ~= ""
            then
                sourceTexture = knifeTextures:FindFirstChild(textureName)
            end
...
--========================================================--
-- ÉCOUTEUR THROWN KNIFE
-- Protégé contre les doubles connexions lors d'un rerun.
--========================================================--

Runtime.ThrowListenerToken = {}
_G.rioV7ThrowListenerToken = Runtime.ThrowListenerToken

trackConnection(workspace.ChildAdded:Connect(function(child)

    -- Une ancienne instance du script peut encore avoir sa
    -- connexion physique. Elle devient immédiatement inactive.
    if _G.rioV7ThrowListenerToken ~= Runtime.ThrowListenerToken then
        return
    end

    if child.Name ~= "ThrownKnife"
        or not child:IsA("Model")
    then
        return
    end
    ...
    task.spawn(function()
        ...
        -- Laisse le jeu construire complètement le projectile.
        ...
        if not child.Parent or not centerPart then
            return
        end

        -- Protection contre une double injection sur le même projectile.
        if child:GetAttribute("rioVFXInjected") then
            return
        end

        child:SetAttribute("rioVFXInjected", true)
...
local function applyAllyHighlight(entity)
    ...
    local hl = char:FindFirstChild("rioAllyHighlight")
    if not hl then
...
local function clearAllyHighlight(entity)
    ...
    local ally = char:FindFirstChild("rioAllyHighlight")
...
local function cleanupAllTracers()
    for entity, tracer in pairs(CTX.activeTracers) do
        pcall(function()
            tracer.Visible = false
            tracer:Remove()
        end)
        CTX.activeTracers[entity] = nil
    end
end
...
CTX.ESPTab:AddDropdown({ Name = "Type ESP", Default = "Désactivé", Options = {"Highlight", "Désactivé"}, Callback = function(v) _G.ESPMode = v end })
CTX.ESPTab:AddToggle({ Name = "Tracers", Default = false, Callback = function(v) _G.TracersEnabled = v if not v then cleanupAllTracers() end end })

CTX.ESPTab:AddDropdown({
    Name = "Origine des tracers",
    Default = "Bas milieu",
    Options = {"Bas milieu", "Centre"},
    Callback = function(v)
        CTX.TracerOrigin = v
    end,
})
